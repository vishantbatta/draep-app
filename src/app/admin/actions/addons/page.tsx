...
export default function AddonsActionPage() {
  return (
    <Suspense fallback={null}>
      <AddonsActionPageInner />
    </Suspense>
  );
}

function AddonsActionPageInner() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [activeActionTab] = useState<ActionTabKey>("addons");
  ...
  // ─── Addon variations for dropdown + relation map (default_variation_id) ─
  const [addonVariations, setAddonVariations] = useState<{ id: string; label: string }[]>([]);
  const [addonVariationMap, setAddonVariationMap] = useState<RelationMap>({});
  ...
  useEffect(() => {
    ...
    fetchTableData("garment_addon_variations", 1, 100)
      .then((d) => {
        setAddonVariations(
          d.rows.map((r) => {
            const labels = r.labels as Record<string, string> | null;
        ...
        setAddonVariationMap(buildRelationMap(d.rows));
      })
      .catch(() => {});
  }, []);
  ...
  const relations: RelationConfig[] = [
    ...
    { column: "default_variation_id", entityLabel: "Default Variation", map: addonVariationMap },
  ];
  ...
  useEffect(() => {
    ...
    fetchTableData("garment_addons", effectivePage, PER_PAGE, effectiveSort)
      .then((d) => {
  ...
  function handleDelete(row: Record<string, unknown>) {
    ...
    const slug = String(row.slug ?? addonId);
    if (!confirm(`Delete add-on "${slug}"? This cannot be undone.`)) return;
    deleteAddon(addonId)
      .then(() => {
  ...
  function rowToAddon(row: Record<string, unknown>): Addon {
    return {
      ...
      garment_style_component_ids: (row.garment_style_component_ids as string[]) ?? null,
      type: (row.type as string) ?? null,
      placements: (row.placements as string[]) ?? null,
      default_variation_id: (row.default_variation_id as string) ?? null,
      is_default_on: (row.is_default_on as boolean) ?? null,
    ...
    <div className="px-4 py-4 md:px-6 md:py-6">
      ...
      {editingRow && (
        <AddonForm
          garments={garments}
          addonVariations={addonVariations}
          existing={rowToAddon(editingRow)}
      ...
      <div className="hidden overflow-x-auto rounded-card border border-hairline bg-chalk-white shadow-card md:block">
        {loading ? (
          ...
          <div className="flex h-48 items-center justify-center">
            <span className="text-caption text-muted">No add-ons yet. Click &quot;Add Add-on&quot; to create one.</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2 md:hidden">
        {loading ? (
          ...
          <div className="flex h-48 items-center justify-center rounded-card border border-hairline bg-chalk-white">
            <span className="text-caption text-muted">No add-ons yet</span>
          </div>
        )}
      </div>
...
function AddonForm({
  ...
  addonVariations: { id: string; label: string }[];
  existing?: Addon;
  onCreated?: () => void;
  onUpdated?: () => void;
  onCancel: () => void;
  ...
  const [defaultVariationId, setDefaultVariationId] = useState(existing?.default_variation_id ?? "");
  const [isDefaultOn, setIsDefaultOn] = useState(
    existing?.is_default_on != null ? String(existing.is_default_on) : "",
  );
  ...
  const [placements, setPlacements] = useState(
    Array.isArray(existing?.placements) ? existing!.placements.join(", ") : "",
  );
  const [gscIds, setGscIds] = useState(
    Array.isArray(existing?.garment_style_component_ids) ? existing!.garment_style_component_ids.join(", ") : "",
  );
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const gscIdsArr = gscIds.split(",").map((s) => s.trim()).filter(Boolean);
    ...
    const commonPayload = {
      ...
      descriptions,
      asset_urls: urls,
      garment_style_component_ids: gscIdsArr.length > 0 ? gscIdsArr : null,
      type: type.trim() || null,
      placements: placementsArr.length > 0 ? placementsArr : null,
      default_variation_id: defaultVariationId || null,
      is_default_on: isDefaultOn === "true" ? true : isDefaultOn === "false" ? false : null,
      price: price.trim() ? parseInt(price, 10) : null,
    };

    if (isEdit && existing && onUpdated) {
      const payload: AddonUpdateInput = commonPayload;
      updateAddon(existing.id, payload)
        .then(() => onUpdated())
        .catch((err) => {
          setFormError(err instanceof Error ? err.message : "Failed to update add-on");
          setSubmitting(false);
        });
    } else if (onCreated) {
      const payload: AddonCreateInput = commonPayload;
      createAddon(payload)
        .then(() => onCreated())
        .catch((err) => {
          setFormError(err instanceof Error ? err.message : "Failed to create add-on");
          setSubmitting(false);
        });
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="mb-5">
        <div className={`rounded-card ${enLabelMissing ? "border border-error-border bg-error-bg/30" : "border border-transparent"} p-1 -m-1`}>
          <LangRowEditor
            ...
            label="Labels *"
            placeholder="e.g. Latkan"
          />
        </div>
      </div>

      {/* Slug + Garment + Type + Priority */}
      <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            className={inputCls}
          >
            <option value="">No garment (global)</option>
            {garments.map((g) => (
      ...
      {/* Default Variation + Is Default On + Price */}
      <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div>
          ...
          <select
            ...
            onChange={(e) => setDefaultVariationId(e.target.value)}
            className={inputCls}
          >
        ...
        <div>
          ...
          <select
            value={isDefaultOn}
            onChange={(e) => setIsDefaultOn(e.target.value)}
            className={inputCls}
          >
            <option value="">—</option>
        ...
        <div>
          ...
          <input
            ...
            className={inputCls}
          />
          <p className="mt-1 text-[11px] text-muted">Only when no variations</p>
        </div>
      </div>
      ...
      <div className="mb-5">
        <LangRowEditor
          ...
          label="Descriptions"
          multiline
          placeholder="e.g. Decorative latkan add-on…"
        />
      </div>

      {/* Placements + Component IDs */}
      <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label className="mb-1.5 block font-mono text-eyebrow text-ink-navy">
            Placements
          ...
          <input
            ...
            placeholder="neck, sleeve (comma-separated)"
            className={inputCls}
          />
          <p className="mt-1 text-[11px] text-muted">Comma-separated placement strings</p>
        </div>
        <div>
          <label className="mb-1.5 block font-mono text-eyebrow text-ink-navy">
            Style Component IDs
          ...
          <input
            type="text"
            value={gscIds}
            onChange={(e) => setGscIds(e.target.value)}
            placeholder="uuid1, uuid2 (comma-separated)"
            className={inputCls}
          />
          <p className="mt-1 text-[11px] text-muted">Comma-separated component UUIDs</p>
        </div>
      </div>
