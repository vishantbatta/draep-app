...
export default function GarmentsActionPage() {
  ...
  function handleDelete(row: Record<string, unknown>) {
    const garmentId = String(row.id ?? "");
    const slug = String(row.slug ?? garmentId);
    if (!confirm(`Delete garment "${slug}"? This cannot be undone.`)) return;
  ...
  // Convert a table row to a Garment object for the edit form
  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-6">
      ...
      {editingRow && (
        <EditGarmentForm
          garment={rowToGarment(editingRow)}
                  ...
                  <tr key={idx} className="border-b border-hairline transition last:border-b-0 hover:bg-warm-sand">
                    {data.columns.map((col) => (
                      <td key={col} className="whitespace-nowrap px-3 py-2 text-data text-ink">
                        <SmartCell column={col} value={row[col]} />
                      </td>
                    ))}
      ...
      <div className="space-y-2 md:hidden">
        {loading ? (
          ...
          data.rows.map((row, idx) => {
            ...
            return (
              <div key={idx}>
                <SmartMobileCard row={row} columns={data.columns} />
          ...
          <div className="flex h-48 items-center justify-center rounded-card border border-hairline bg-chalk-white">
            <span className="text-caption text-muted">No garments yet</span>
          </div>
        )}
      </div>
...
/** Generate a slug from the English label: name_random_<8-char-uuid>. */
...
/** Convert a labels/descriptions dict to LangRow[] for the editor. */
function dictToLangRows(dict: Record<string, string> | null): LangRow[] {
  if (!dict) return [];
  return Object.entries(dict).map(([lang, value]) => newLangRow(lang === "" ? "" : lang, value));
}
...
function EditGarmentForm({
  ...
  const [slug, setSlug] = useState(garment.slug ?? "");
  const [gender, setGender] = useState(garment.gender ?? "");
  const [basePrice, setBasePrice] = useState(
    garment.base_price != null ? String(garment.base_price) : "",
  );
  const [labelRows, setLabelRows] = useState<LangRow[]>(() => {
    const rows = dictToLangRows(garment.labels);
    // Ensure English exists
    if (!rows.some((r) => r.lang === "en")) {
      rows.unshift(newLangRow("en"));
    }
    return rows;
  });
  const [descRows, setDescRows] = useState<LangRow[]>(() =>
    dictToLangRows(garment.descriptions),
  );
  const [assetUrls, setAssetUrls] = useState(
    Array.isArray(garment.asset_urls) ? garment.asset_urls.join("\n") : "",
  );
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!basePrice.trim()) {
      ...
      if (isNaN(parsed) || parsed < 0) errors.push("Base price must be a valid positive number.");
    }
    ...
    // Build payload
    const payload: GarmentUpdateInput = {
      slug: slug.trim() || autoSlug(enLabelValue),
      gender: gender.trim(),
    ...
    setSubmitting(true);
    updateGarment(garment.id, payload)
      .then(() => onUpdated())
      .catch((err) => {
        setFormError(err instanceof Error ? err.message : "Failed to update garment");
        setSubmitting(false);
      });
  }
  ...
  const genderMissing = showErrors && !gender.trim();
  const priceMissing = showErrors && !basePrice.trim();
  ...
  return (
    <form
      ...
      className="mb-4 rounded-card border border-accent-text/30 bg-chalk-white p-4 shadow-card md:p-6"
    >
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div>
          <label className="mb-1.5 block font-mono text-eyebrow text-ink-navy">Gender *</label>
          <select
            value={gender}
            onChange={(e) => setGender(e.target.value)}
            className={`${inputCls} ${genderMissing ? "border-error-border" : ""}`}
          >
        ...
        <div>
          <label className="mb-1.5 block font-mono text-eyebrow text-ink-navy">Base Price *</label>
          <input
            type="number"
            value={basePrice}
      ...
      <div className="mb-5">
        <div className={`rounded-card ${noDescription ? "border border-error-border bg-error-bg/30" : "border border-transparent"} p-1 -m-1`}>
          <LangRowEditor
            ...
            label="Descriptions *"
            multiline
            placeholder="e.g. A custom-tailored silk blouse…"
          />
        </div>
      </div>
      ...
      {/* Asset URLs */}
      <div className="mb-5">
        <label className="mb-1.5 block font-mono text-eyebrow text-ink-navy">Asset URLs *</label>
        <textarea
          value={assetUrls}
...
function AddGarmentForm({
  ...
  const [basePrice, setBasePrice] = useState("");
  const [labelRows, setLabelRows] = useState<LangRow[]>([newLangRow("en")]);
  const [descRows, setDescRows] = useState<LangRow[]>([]);
  const [assetUrls, setAssetUrls] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);

  // Auto-generate slug from English label unless user edited slug manually
  const enLabel = labelRows.find((r) => r.lang === "en")?.value ?? "";
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    // 1. English label is required
    const enLabelValue = labelRows.find((r) => r.lang === "en")?.value.trim() ?? "";
    if (!enLabelValue) {
      errors.push("English label is required.");
    }

    // 2. At least one description required
    const hasDescription = descRows.some((r) => r.value.trim());
    if (!hasDescription) {
    ...
    // 4. Base price required and must be a valid number
    if (!basePrice.trim()) {
      errors.push("Base price is required.");
    } else {
      const parsed = parseInt(basePrice, 10);
    ...
    // 5. At least one asset URL required
    const urls = assetUrls
      .split("\n")
      .map((s) => s.trim())
      .filter(Boolean);
    if (urls.length === 0) {
      errors.push("At least one asset URL is required.");
    }
    ...
    // ── Build payload ───────────────────────────────────────────────────
    // Slug: auto-generate if user hasn't manually set one
    let finalSlug = slug.trim();
    if (!finalSlug) {
      finalSlug = autoSlug(enLabelValue);
    }
    ...
    // Build descriptions dict from rows
    const descriptions: Record<string, string> = {};
    for (const row of descRows) {
      if (row.lang && row.value.trim()) descriptions[row.lang] = row.value.trim();
    }
    ...
    createGarment(payload)
      .then(() => onCreated())
      .catch((err) => {
        setFormError(err instanceof Error ? err.message : "Failed to create garment");
        setSubmitting(false);
      });
  }
  ...
  return (
    <form
      onSubmit={handleSubmit}
      className="mb-4 rounded-card border border-hairline bg-chalk-white p-4 shadow-card md:p-6"
    >
      <h2 className="mb-4 font-heading text-body font-semibold text-ink-navy">
        New Garment
      </h2>
      ...
      <div className="mb-5">
        <div className={`rounded-card ${enLabelMissing ? "border border-error-border bg-error-bg/30" : "border border-transparent"} p-1 -m-1`}>
          <LangRowEditor
            rows={labelRows}
            onChange={handleEnLabelChange}
            label="Labels *"
            placeholder="e.g. Silk Blouse"
          />
          {enLabelMissing && (
            <p className="mt-1 px-2 text-[11px] text-error-text">
      ...
      <div className="mb-5">
        ...
        {noAssetUrl && (
          <p className="mt-1 text-[11px] text-error-text">
            At least one asset URL is required.
          </p>
        )}
      </div>
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="tap rounded-pill bg-ink-navy px-6 py-2.5 text-caption font-medium text-chalk-white transition hover:bg-ink-navy/90 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {submitting ? "Creating…" : "Create Garment"}
        </button>
        <button
          type="button"
          onClick={onCancel}
      ...
      {/* Debug preview */}
      {slug && (
        <div className="mt-4 rounded-card bg-warm-sand/50 px-3 py-2 font-mono text-[11px] text-muted">
          slug: {slug}
        </div>
      )}
    </form>
...
// ═══════════════════════════════════════════════════════════════════════════════
//  Shared components (sortable header, cell renderer, mobile card)
// ═══════════════════════════════════════════════════════════════════════════════
