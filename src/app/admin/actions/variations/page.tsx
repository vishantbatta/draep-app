...
export default function VariationsActionPage() {
  return (
    <Suspense fallback={null}>
      <VariationsActionPageInner />
    </Suspense>
  );
}

function VariationsActionPageInner() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [activeActionTab] = useState<ActionTabKey>("variations");
  ...
  // ─── Style components for dropdown + relation map ────────────────────────
  const [components, setComponents] = useState<{ id: string; label: string }[]>([]);
  const [componentMap, setComponentMap] = useState<RelationMap>({});
  const [variationTypeMap, setVariationTypeMap] = useState<RelationMap>({});

  useEffect(() => {
    fetchTableData("garment_style_component", 1, 100)
      .then((d) => {
        setComponents(
          d.rows.map((r) => {
        ...
        setComponentMap(buildRelationMap(d.rows));
      })
      .catch(() => {});

    // Fetch variation types for default_type_id relation
    fetchTableData("garment_style_component_variation_types", 1, 100)
      .then((d) => {
        setVariationTypeMap(buildRelationMap(d.rows));
      })
      .catch(() => {});
  }, []);

  const relations: RelationConfig[] = [
    { column: "component_id", entityLabel: "Style Component", map: componentMap },
    { column: "default_type_id", entityLabel: "Default Variation Type", map: variationTypeMap },
  ];
  ...
  useEffect(() => {
    ...
    fetchTableData("garment_style_component_variations", effectivePage, PER_PAGE, effectiveSort)
      .then((d) => {
  ...
  function handleDelete(row: Record<string, unknown>) {
    const varId = String(row.id ?? "");
    const slug = String(row.slug ?? varId);
    if (!confirm(`Delete variation "${slug}"? This cannot be undone.`)) return;
  ...
  function rowToVariation(row: Record<string, unknown>): Variation {
    return {
      id: String(row.id ?? ""),
      ...
      default_type_id: (row.default_type_id as string) ?? null,
    };
  }
  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-6">
      ...
      {editingRow && (
        <VariationForm
          components={components}
          existing={rowToVariation(editingRow)}
      ...
      <div className="hidden overflow-x-auto rounded-card border border-hairline bg-chalk-white shadow-card md:block">
        {loading ? (
          ...
          <div className="flex h-48 items-center justify-center">
            <span className="text-caption text-muted">No variations yet. Click &quot;Add Variation&quot; to create one.</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2 md:hidden">
        {loading ? (
          ...
          <div className="flex h-48 items-center justify-center rounded-card border border-hairline bg-chalk-white">
            <span className="text-caption text-muted">No variations yet</span>
          </div>
        )}
      </div>
...
function VariationForm({
  ...
  const [componentId, setComponentId] = useState(existing?.component_id ?? "");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!componentId) errors.push("Component is required.");
    ...
    if (isEdit && existing && onUpdated) {
      const payload: VariationUpdateInput = {
        slug: finalSlug,
      ...
      updateVariation(existing.id, payload)
        .then(() => onUpdated())
        .catch((err) => {
          setFormError(err instanceof Error ? err.message : "Failed to update variation");
          setSubmitting(false);
        });
    } else if (onCreated) {
      const payload: VariationCreateInput = {
        slug: finalSlug,
        component_id: componentId,
        priority_order: parseInt(priorityOrder, 10),
        price: parseInt(price, 10),
      ...
      createVariation(payload)
        .then(() => onCreated())
        .catch((err) => {
          setFormError(err instanceof Error ? err.message : "Failed to create variation");
          setSubmitting(false);
        });
    }
  }
  ...
  const componentMissing = showErrors && !componentId;
    ...
    >
      <div className="mb-4 flex items-center gap-2">
        <h2 className="font-heading text-body font-semibold text-ink-navy">
          {isEdit ? "Edit Variation" : "New Variation"}
        </h2>
        {isEdit && existing && (
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            className={`${inputCls} ${componentMissing ? "border-error-border" : ""}`}
          >
      ...
      <div className="mb-5">
        <div className={`rounded-card ${noDescription ? "border border-error-border bg-error-bg/30" : "border border-transparent"} p-1 -m-1`}>
          <LangRowEditor
            ...
            label="Descriptions *"
            multiline
            placeholder="e.g. A full-length sleeve option…"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {submitting ? "Saving…" : isEdit ? "Save Changes" : "Create Variation"}
        </button>
        <button
          type="button"
          onClick={onCancel}
